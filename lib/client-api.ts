...
import {
  ...
  deletePostExtended as deleteFirebasePost,
  ...
  recordViewExtended as recordFirebaseView,
  ...
  updatePostExtended as updateFirebasePost,
} from "./firebase-db"
...
const mockPosts: Post[] = [
  {
    id: "1",
    title:
      "В ИИ-сервисе вместо искусственного интеллекта работали сотни операторов",
    content:
  ...
  },
  {
    ...
    content:
      "Недавно OpenAI представила обновление для GPT-4, которое значительно расширяет возможности этой модели в контексте веб-разработки. В этой статье мы рассмотрим ключевые улучшения и как они могут быть использованы для оптимизации рабочего процесса.",
    author: {
...
export async function getPaginatedPosts(options: {
  ...
  if (!isBrowser) {
    ...
    const startIndex = startAfter
      ? filteredPosts.findIndex((post) => post.id === startAfter) + 1
      : 0
...
export async function getPostsByCategory(category: string): Promise<Post[]> {
  ...
  if (!isBrowser) {
    return mockPosts.filter(
      (post) => post.category === category && !post.archived,
    )
  }
  ...
  try {
    ...
    console.error(
      `Ошибка при получении постов по категории ${category}:`,
      error,
    )
    return []
  }
...
export async function getPostsByAuthor(authorId: string): Promise<Post[]> {
  ...
  if (!isBrowser) {
    return mockPosts.filter(
      (post) => post.author_id === authorId && !post.archived,
    )
  }
...
export async function getPostsByTag(tag: string): Promise<Post[]> {
  ...
  if (!isBrowser) {
    return mockPosts.filter(
      (post) => post.tags?.includes(tag) && !post.archived,
    )
  }
