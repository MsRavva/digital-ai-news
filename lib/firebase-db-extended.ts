import type { Comment, Post, PostStats, Tag } from "@/types/database"
...
import { convertTimestampToISO, db } from "./firebase-db-core"
...
export async function getPaginatedPosts(options: {
  ...
  try {
    ...
    const filteredDocs = postsSnapshot.docs.filter((doc) =>
      filteredPostIds.includes(doc.id),
    )
    ...
    const lastVisible =
      filteredDocs.length > 0 ? filteredDocs[filteredDocs.length - 1].id : null
