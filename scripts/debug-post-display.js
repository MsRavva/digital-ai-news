  ...
  try {
    ...
    if (postData.author_id) {
      ...
      if (authorDoc.exists) {
        ...
        console.log('\nАвтор публикации не найден (ID:', postData.author_id, ')');
      }
    } else {
    ...
    const nonArchivedQuery = await db.collection('posts')
    ...
    const foundInNonArchived = nonArchivedPosts.find(post => post.id === postId);
    console.log('- Найдена среди неархивированных публикаций:', foundInNonArchived ? 'Да' : 'Нет');
    
    // 5. Проверяем, как публикация выглядит в запросе с фильтрацией по категории
    const categoryQuery = await db.collection('posts')
    ...
    console.log('- Найдена среди публикаций категории', postData.category + ':', foundInCategory ? 'Да' : 'Нет');
    ...
    const viewsQuery = await db.collection('views')
      .where('post_id', '==', postId)
      .get();
