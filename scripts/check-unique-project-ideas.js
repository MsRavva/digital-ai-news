...
function getIdeaDescription(text) {
  const descMatch = text.match(/##\s*Введение\s+(.+?)(?=\s*##|\s*$)/is)
  ...
  newIdeas.forEach((idea) => {
    ...
    if (firebaseTitles.includes(title) || fileTitles.includes(title)) {
      ...
      for (const fileIdea of fileIdeas) {
        const commonKeywords = fileIdea.keywords.filter((k) =>
          keywords.includes(k),
        )
        const themeMatch =
          fileIdea.theme &&
          getIdeaTheme(idea) &&
          fileIdea.theme === getIdeaTheme(idea)
        const descSim =
          fileIdea.description &&
          getIdeaDescription(idea) &&
          fileIdea.description.includes(getIdeaDescription(idea))
...
if (newIdeas.length === 0) {
  console.log(
    "Укажите новые идеи для проверки через аргументы командной строки.",
  )
  process.exit(0)
}
