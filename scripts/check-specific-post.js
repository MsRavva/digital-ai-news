...
async function checkSpecificPost(postId) {
  ...
  try {
    ...
    console.log(
      `- created_at: ${postData.created_at ? "присутствует" : "отсутствует"}`,
    )
    console.log(
      `- updated_at: ${postData.updated_at ? "присутствует" : "отсутствует"}`,
    )
    console.log(
      `- likesCount: ${postData.likesCount !== undefined ? postData.likesCount : "отсутствует"}`,
    )
    console.log(
      `- commentsCount: ${postData.commentsCount !== undefined ? postData.commentsCount : "отсутствует"}`,
    )
    console.log(
      `- viewsCount: ${postData.viewsCount !== undefined ? postData.viewsCount : "отсутствует"}`,
    )
    ...
    if (currentUserId) {
      ...
      if (userDoc.exists) {
        ...
        console.log(
          `- Автор публикации: ${postData.author_id === currentUserId ? "Да" : "Нет"}`,
        )
