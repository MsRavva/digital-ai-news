...
async function fixAllPosts() {
  try {
    ...
    for (const postDoc of postsSnapshot.docs) {
      ...
      const hasArchivedIssue =
        postData.archived === undefined || postData.archived === null
      const hasDateIssue =
        !postData.created_at ||
        (postData.created_at.toDate &&
          postData.created_at.toDate() > new Date()) ||
        (postData.created_at._seconds &&
          postData.created_at._seconds > Math.floor(Date.now() / 1000))
    ...
    if (fixedPosts.length > 0) {
      ...
      fixedPosts.forEach((post, index) => {
        ...
        if (post.issues.updated_at)
          console.log("   - Исправлена дата обновления")
      })
    }
