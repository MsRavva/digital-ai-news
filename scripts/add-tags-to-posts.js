...
const projectIdeasTags = [
  'ИИ', 'Веб-разработка', 'Образование', 'Машинное обучение', 
  'Персонализация', 'Генеративные модели', 'Анализ данных', 
  'Мобильные приложения', '3D-моделирование', 'Компьютерное зрение'
];
  ...
  try {
    ...
    for (const tagName of projectIdeasTags) {
      ...
      if (tagsSnapshot.empty) {
        ...
        console.log(`Использован существующий тег: ${tagName} (ID: ${tagsSnapshot.docs[0].id})`);
      }
    }
    ...
    for (const doc of snapshot.docs) {
      ...
      console.log(`Добавляем ${numTags} тегов к публикации: ${postData.title} (ID: ${postId})`);
      ...
      for (const tagName of selectedTags) {
        ...
        if (postTagsSnapshot.empty) {
          ...
          console.log(`  - Тег "${tagName}" (ID: ${tagId}) уже связан с публикацией`);
        }
      }
