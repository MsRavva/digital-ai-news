...
async function getPaginatedPosts(options = {}) {
  try {
    ...
    const lastVisible = postsSnapshot.docs.length > 0 ? postsSnapshot.docs[postsSnapshot.docs.length - 1].id : null;
  ...
  try {
    ...
    if (postDoc.exists) {
      ...
      if (postData.category === 'news') {
        console.log('Категория "news" - должна отображаться при фильтре по этой категории');
      } else {
        console.log(`Категория "${postData.category}" - не будет отображаться при фильтре по категории "news"`);
      }

      if (postData.archived) {
        console.log('Публикация архивирована - не будет отображаться при includeArchived=false');
      } else {
        console.log('Публикация не архивирована - должна отображаться при includeArchived=false');
      }

      // Проверяем теги
      const postTagsSnapshot = await db.collection('post_tags').where('post_id', '==', postId).get();
