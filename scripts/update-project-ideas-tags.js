...
function extractTitleFromFile(filePath) {
  try {
    ...
    console.error(`Ошибка при извлечении заголовка из файла ${filePath}:`, error);
    return null;
  }
}
...
async function updateProjectIdeaTags(postId, tags) {
  try {
    ...
    console.error(`Ошибка при обновлении тегов для публикации ${postId}:`, error);
    return false;
  }
}
...
async function updateAllProjectIdeasTags() {
  try {
    ...
    console.log(`Создана карта заголовков и тегов для ${titleToTagsMap.size} идей проектов`);
    ...
    snapshot.forEach(doc => {
      ...
      if (titleToTagsMap.has(title)) {
        ...
        updateProjectIdeaTags(doc.id, tags)
          .then(success => {
            if (success) {
              updatedCount++;
            }
