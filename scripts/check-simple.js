...
async function checkPost() {
  try {
    ...
    if (postDoc.exists) {
      ...
      const newsQuery = await db.collection('posts')
        .where('category', '==', 'news')
      ...
      console.log(`Публикация ${postId} ${found ? 'найдена' : 'не найдена'} в результатах запроса`);
      
      if (!found && data.category === 'news') {
        console.log('ПРОБЛЕМА: Публикация должна быть в результатах, но её нет!');
      }
    } else {
