...
function extractContent($: cheerio.CheerioAPI) {
  ...
  const contentSelectors = [
    ...
    '.content', 'main', '#content', '.post-body', '.story-body'
  ];
  ...
  for (const selector of contentSelectors) {
    ...
    if (element.length > 0) {
      ...
      element.find('script, style, iframe, .comments, .related, .share, .ads, .advertisement').remove();
...
function convertHtmlToText($: cheerio.CheerioAPI, html: string) {
  ...
  $content('ul, ol').each((i, el) => {
    const items = $content(el).find('li').map((i, li) => {
...
async function extractImages($: cheerio.CheerioAPI, baseUrl: string) {
  ...
  const imgElements = $('article img, .content img, .post-content img, main img, .entry-content img').toArray();
...
function extractTags($: cheerio.CheerioAPI) {
  ...
  const tagSelectors = [
    '.tags a', '.post-tags a', '.article-tags a', '.entry-tags a', 
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user || (user.role !== 'teacher' && user.role !== 'admin' && user.uid !== '4J9Vf4tqKOU7vDcz99h6nBu0gHx2')) {
