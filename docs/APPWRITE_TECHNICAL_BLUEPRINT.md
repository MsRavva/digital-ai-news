...
- Статус: phase 1 completed / ready for read-path migration
- Дата фиксации: 2026-04-23
- Основание: подтвержденный scope миграции с Supabase на Appwrite
...
- Пока `NEXT_PUBLIC_BACKEND_PROVIDER` не установлен в `appwrite`, приложение использует Supabase через внутренние сервисы.
- Appwrite schema и SDK helpers готовы для следующего этапа, но auth/read/write runtime не переключены.
- Production-переключение на Appwrite допускается только после реализации Appwrite read adapters, session endpoints, middleware integration и smoke-test.
...
- Blueprint зафиксирован в документации.
- `memory_bank` синхронизирован с новым scope.
- В коде появляется provider-agnostic service layer.
- В проекте добавлены каркасные Appwrite helpers без полного cutover.
- Appwrite TablesDB schema создана через `scripts/setup-appwrite-schema.ts`.
