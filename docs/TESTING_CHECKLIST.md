...
- [ ] OAuth провайдеры (Google, GitHub) настроены в Appwrite Console
- [ ] В Google Cloud Console добавлен точный Appwrite redirect URI: `https://<region>.cloud.appwrite.io/v1/account/sessions/oauth2/callback/google/<project-id>`
- [ ] Приложение запущено (`bun run dev`)
...
- [ ] Проверить, что пользователь создан в Appwrite Console → Auth → Users
...
### Проверка профиля после регистрации
- [ ] Зарегистрировать нового пользователя
- [ ] Проверить в Appwrite Console → TablesDB → profiles
- [ ] Убедиться, что профиль создан автоматически
- [ ] Проверить, что поля заполнены корректно:
  - `id` = Appwrite account id
