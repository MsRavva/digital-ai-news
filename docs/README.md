...
- Для корректного link preview GET/HEAD-запросы известных crawler user agents к `/posts/[id]` пропускаются до route metadata без auth redirect.
...
- `/posts/[id]` - просмотр отдельной публикации; динамическая metadata/OpenGraph-разметка формируется из заголовка, текста и первого Markdown-изображения статьи для корректного link preview.
