...
- Страница `/login` поддерживает диагностический OAuth режим: справа от формы показывается пошаговый трекер, а при debug-flow callback возвращает пользователя обратно на `/login` перед финальным редиректом.
- Для диагностического режима клиент получает provider URL через `skipBrowserRedirect: true`, сам вызывает `window.location.assign(...)` и показывает ручную ссылку, если браузер не ушел на провайдера.
