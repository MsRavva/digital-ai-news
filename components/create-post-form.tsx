...
import { LinkIcon, Loader2, X } from "lucide-react"
...
export function CreatePostForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Ошибка",
        description:
          error.message || "Произошла ошибка при создании публикации",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  const renderPreview = () => {
    ...
    if (previewData.attachments.length > 0) {
      const attachmentsContent = previewData.attachments
        .map((a) => {
    ...
    // Форматируем дату
    const formattedDate = new Date(previewData.created_at).toLocaleDateString(
      "ru-RU",
      {
        day: "numeric",
        month: "long",
            ...
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <CardDescription>
                    Создайте новую публикацию для обмена новостями, ссылками или
                    файлами.
                  </CardDescription>
                </div>
                <div className="flex gap-2">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <MDEditor
                  value={content}
                  onChange={(value) => {
                    console.log("Create form MDEditor onChange called")
                    setContent(value || "")
                  }}
                />
              </div>
