    ...
    mdParser.renderer.rules.fence = (tokens, idx, options, env, self) => {
      ...
      return `
        <div class="code-block-wrapper">
          <pre class="relative overflow-x-auto rounded-md bg-slate-50 dark:bg-slate-900 text-sm text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-800 my-2">
            ${lang ? `<span class="absolute left-2 top-1 z-10 rounded-md bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-xs font-medium text-slate-600 dark:text-slate-400 border border-slate-300 dark:border-slate-700">${lang}</span>` : ''}
            <button
              class="copy-button absolute right-3 bottom-1 z-10 rounded-sm p-1 bg-[hsl(var(--saas-purple))] text-white border border-[hsl(var(--saas-purple-dark))] shadow-sm transition-all hover:bg-[hsl(var(--saas-purple-dark))] hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[hsl(var(--saas-purple))]/50"
            ...
            >
              ...
              <svg class="check-icon h-3.5 w-3.5 hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
            </button>
            <code class="block px-4 py-2 pt-6 ${lang ? `language-${lang}` : ''}">${escapeHtml(code)}</code>
          </pre>
        </div>
      `;
    };
